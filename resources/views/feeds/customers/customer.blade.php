                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        <div class="col-md-4 col-print-12">
                            ...
                            <h4><strong>Total Sales : <span class="navy">Tk.{{number_format(DB::table('feed_sales')->where('feed_customer_id', $customer->id)->sum('total') + $customer->balance, 2)}}</span></strong></h4>
                            <h4><strong>Total Payment : <span class="navy">Tk.{{number_format(DB::table('feed_sales')->where('feed_customer_id', $customer->id)->sum('payment') + $customer->payment, 2)}}</span></strong></h4>
                            <h4><strong>Balnce/Dues : <span class="red">Tk.{{round(DB::table('feed_sales')->where('feed_customer_id', $customer->id)->sum('total') + $customer->balance -DB::table('feed_sales')->where('feed_customer_id', $customer->id)->sum('payment') - $customer->payment, 2) }}</span></strong></h4>
                        </div>
                        ...
                        <table class="table table-bordered table-striped table-hover"  id="list-sale-report">
                            ...
                            @foreach($sales as $sale)
                                <tr>
                                    ...
                                    <td class="hidden-print">{{$sale->feed_supplier->supplier_name}}</td>
                                    <td>{{$sale->qty}}</td>
...
@section('scripts')
    <script type="text/javascript">
        ...
        function listSales(criteria1, criteria2, criteria3)
        {
            $.ajax({
                type : 'get',
                url : "{!! url('/feeds/customers/getsales') !!}",
